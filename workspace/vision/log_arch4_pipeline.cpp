// log_arch4_pipeline.cpp
// Architecture 4 — MPI Distributed Pipeline: stage-parallel LoG across ranks.
// Rank 0: LoG convolution
// Rank 1: Zero-crossing detection + save output
// In streaming mode, rank 0 processes image i+1 while rank 1 processes image i.
// Usage: mpirun -n 2 ./log_arch4 <image_list_file> <output_dir> <N_images>
//        mpirun -n 2 ./log_arch4 <single_image> <output.png>   (single-image mode, argc==3)
#include <mpi.h>
...
int main(int argc, char** argv) {
    ...
    if (size < 2) {
        if (rank == 0) fprintf(stderr, "[LoG Pipeline] Need at least 2 ranks.\n");
        MPI_Finalize();
        return 1;
    }

    // Single-image mode: log_arch4 <input> <output>
    ...
    if (rank == 0) {
        ...
        for (int img_id = 0; img_id < N_images; ++img_id) {
            ...
            // LoG convolution
            std::vector<float> resp(w * h, 0.0f);
            for (int y = 0; y < h; ++y)
                for (int x = 0; x < w; ++x) {
                    float r = 0.0f;
                    for (int ky = -2; ky <= 2; ++ky) {
                        ...
                        for (int kx = -2; kx <= 2; ++kx) {
                            ...
                            r += img[ny*w + nx] * K_LOG[ky+2][kx+2];
                        }
                    }
                    resp[y*w + x] = r;
                }

            MPI_Send(resp.data(), w * h, MPI_FLOAT, 1, 0, MPI_COMM_WORLD);
            stbi_image_free(img);
        }
        ...
        // ---- Stage 1: Zero-crossing detection + save ----
        int img_id = 0;
        while (true) {
            int dims[2];
            ...
            std::vector<float> resp(w * h);
            MPI_Recv(resp.data(), w * h, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            std::vector<unsigned char> out(w * h, 0);
            for (int y = 1; y < h - 1; ++y)
                for (int x = 1; x < w - 1; ++x) {
                    float cv = resp[y*w + x];
                    if ((cv * resp[y*w + x + 1]   < 0) ||
                        (cv * resp[y*w + x - 1]   < 0) ||
                        (cv * resp[(y+1)*w + x]   < 0) ||
                        (cv * resp[(y-1)*w + x]   < 0))
                        out[y*w + x] = 255;
            ...
            if (single_mode) {
                ...
                snprintf(out_path, sizeof(out_path), "%s/log_out_%04d.png", argv[2], img_id++);
            }
    ...
    if (rank == 0) {
        printf("[LoG MPI Pipeline] N=%d  total_time=%.5f s  throughput=%.3f img/s\n",
